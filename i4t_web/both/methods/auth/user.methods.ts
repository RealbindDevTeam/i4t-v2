...
if( Meteor.isServer ){
    Meteor.methods({
        penalizeCustomer: function( _pCustomerUser : User ){
            ...
            let _lCustomerAccount: Account = Accounts.findOne( { establishment_id: _lCustomerEstablishment, tableId: _lCustomerTable, status: 'OPEN' } );

            if( _lCustomerAccount ){
                let _lUserOrders: string [] = [];
                Orders.find( { creation_user: _pCustomerUser._id, establishment_id: _lCustomerEstablishment, tableId: _lCustomerTable, accountId: _lCustomerAccount._id } ).fetch().forEach( ( order ) => {
                    _lUserOrders.push( order._id );
                });
                Orders.find( { creation_user: _pCustomerUser._id, establishment_id: _lCustomerEstablishment, tableId: _lCustomerTable, accountId: _lCustomerAccount._id,
                ...
                Orders.find( { establishment_id: _lCustomerEstablishment, tableId: _lCustomerTable, accountId: _lCustomerAccount._id, 'translateInfo.firstOrderOwner': _pCustomerUser._id, 
                ...
                Orders.find( { establishment_id: _lCustomerEstablishment, tableId: _lCustomerTable, accountId: _lCustomerAccount._id, 'translateInfo.lastOrderOwner': _pCustomerUser._id, 
                'translateInfo.markedToTranslate': true, status: 'ORDER_STATUS.PENDING_CONFIRM', toPay : false } ).fetch().forEach( ( order ) => {
                ...
                WaiterCallDetails.find( { establishment_id: _lCustomerEstablishment, table_id: _lCustomerTable, order_id: { $in: _lUserOrders }, type: 'SEND_ORDER',
                ...
                WaiterCallDetails.find( { establishment_id: _lCustomerEstablishment, table_id: _lCustomerTable, user_id: _pCustomerUser._id, type: { $in : [ 'CALL_OF_CUSTOMER', 'USER_EXIT_TABLE', 'PAYMENT' ] },
                status: { $in: [ 'waiting', 'completed' ] } } ).fetch().forEach( ( call ) => {
                    WaiterCallDetails.update( { _id: call._id }, { $set: { status: 'cancel', modification_date : new Date() } } );
                });

                Payments.find( { creation_user: _pCustomerUser._id, establishment_id: _lCustomerEstablishment, accountId: _lCustomerAccount._id,
