...
export class OrderAttentionComponent implements OnInit, OnDestroy {
    ...
    private _userEstablishmentSub: Subscription;
    ...
    ngOnInit() {
        ...
        this._userDetailSub = MeteorObservable.subscribe('getUserDetailsByUser', Meteor.userId()).subscribe(() => {
            ...
            if (this._userDetail) {
                this._userEstablishmentSub = MeteorObservable.subscribe('getEstablishmentById', this._userDetail.establishment_work).subscribe(() => {
                    this._establishments = Establishments.find({ _id: this._userDetail.establishment_work });
                });
            }
        });

        this._ordersSub = MeteorObservable.subscribe('getOrdersByEstablishmentWork', this._user, ['ORDER_STATUS.IN_PROCESS', 'ORDER_STATUS.CANCELED']).subscribe(() => {
            this._ngZone.run(() => {
    ...
    /**
     * Validate if establishments exist
     */
    countOrders(): void {
    ...
    removeSubscriptions(): void {
        ...
        if (this._userEstablishmentSub) { this._userEstablishmentSub.unsubscribe(); }
    }
